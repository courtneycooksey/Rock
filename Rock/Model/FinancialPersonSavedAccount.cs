    ...
    public partial class FinancialPersonSavedAccount : Model<FinancialPersonSavedAccount>
    {
        ...
        private string GetNameForHistory()
        {
            ...
            if ( FinancialPaymentDetail == null && FinancialPaymentDetailId.HasValue )
            {
                ...
                FinancialPaymentDetail = financialPaymentDetailService.Get( FinancialPaymentDetailId.Value );
            }

            if ( FinancialPaymentDetail != null )
            {
                var expirationMonth = FinancialPaymentDetail.ExpirationMonth;
                var expirationYear = FinancialPaymentDetail.ExpirationYear;
        ...
        /// <returns></returns>
        /// <exception cref="InvalidDataException">No EntityType record was found for {type.Name}</exception>
        private int GetEntityTypeId<T>()
        {
            var type = typeof( T );
            ...
            {
                throw new InvalidDataException( $"No EntityType record was found for {type.Name}" );
            }

            return entityTypeId.Value;
        }
        ...
        /// <returns></returns>
        /// <exception cref="InvalidDataException">No EntityType record was found for {type.Name}</exception>
        private int GetCategoryId( string guidString )
        {
            var categoryId = CategoryCache.GetId( guidString.AsGuid() );

            if ( !categoryId.HasValue )
            {
                throw new InvalidDataException( $"No Category record was found with guid {guidString}" );
            }
